      ...
      {yearlyRentalData && Array.isArray(yearlyRentalData) && yearlyRentalData.length > 0 ? (
        <svg width="100%" height="300" viewBox="0 0 800 300">
          <g className="chart-lines">
            {/* Линия накопительного дохода */}
            <polyline
              className="chart-line"
              points={yearlyRentalData.map((yearData, i) => 
                `${50 + i * 35},${250 - (yearData.cumulativeIncome / Math.max(...yearlyRentalData.map(y => y.cumulativeIncome))) * 200}`
              ).join(' ')}
              fill="none"
              stroke="#3b82f6"
              strokeWidth="2"
            ...
            <g className="line-points">
              {yearlyRentalData.map((yearData, i) => (
                <circle
                  key={i}
                  cx={50 + i * 35}
                  cy={250 - (yearData.cumulativeIncome / Math.max(...yearlyRentalData.map(y => y.cumulativeIncome))) * 200}
                  r="3"
                  fill="#3b82f6"
                />
              ))}
            ...
            <g className="x-labels">
              {yearlyRentalData.map((yearData, i) => (
                <text
                  ...
                  fill="#666"
                >
                  {yearData.year === 0 ? t.keys : yearData.year}
                </text>
              ))}
            </g>
            ...
            <g className="y-labels">
              {[0, 0.25, 0.5, 0.75, 1].map((ratio, i) => {
                const maxIncome = Math.max(...yearlyRentalData.map(y => y.cumulativeIncome));
                const income = maxIncome * ratio;
            ...
            <g className="chart-legend">
              ...
              <text x="620" y="32" fontSize="12" fill="#333">{t.cumulativeIncome}</text>
            </g>
          </g>
        </svg>
      ) : (
